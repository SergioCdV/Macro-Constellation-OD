...
function [J] = milankovitch_jacobian(J2, Keci, s)
    ...
    dn_p = -3 * (h_norm^2 / eta^2)^(-5/2) * (h_norm^2 / eta^2) * uH.';
    dn_e = -3 * (h_norm^2^2 / eta^2)^(-5/2) * (h_norm^2^2 / eta^2) * e.';
    ...
    J(7,4:6) = J(7,4:6) + dn_e + (eta .* (3*zeta.^2-1) + 5 * zeta.^2 - 2 * zeta - 1) * dnp_e / 2;
end