...
% Date: 22/01/2024
% This file provides an EKF implementation for the Milankovitch dynamics
...
t0 = 0;                         % Initial epoch
tf = 10;                         % Final epoch [orbital periods at the Earth's WGS84 radius]
...
%% Offline observation 
% Constants 
Pobs = 1e-4 * eye(7);       % Covariance of the observation 
...
EKF_estimator = EKF_estimator.AssignStateProcess(s_dim, @(Q, time_step, s, sigma)state_model(J2, Keci, Q, time_step, s, sigma));
...
while (i <= size(meas,1))
    ...
    % Correction
    [se, Pe] = EKF_estimator.CorrectionStep(meas(i,2:end).', se, Pe);
...
%% Auxiliary functions 
% Propagation model 
function [s, P] = state_model(J2, Keci, Q, dt, s0, P0)
